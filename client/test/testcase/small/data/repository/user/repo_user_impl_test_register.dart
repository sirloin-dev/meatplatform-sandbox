...
void registerTestCases(
  ...
  setUp(() {
    ...
    when(userApi.createUser(nickname: anyNamed("nickname"), profileImageUrl: anyNamed("profileImageUrl")))
      ...
      return UserResponse(uuid: uuid, nickname: realNickname, profileImageUrl: realProfileImageUrl);
    });
    ...
    when(userLocalStorage.saveMyUuid(any)).thenAnswer((realInvocation) async {
  ...
  test("api call 결과를 반환한다", () async {
    // then:
    final actual = await sut.register(nickname: nickname, profileImageUrl: profileImageUrl);
  ...
  test("local cache 저장에 실패하더라도 실행 결과에 문제가 없다", () async {
    ...
    when(userLocalStorage.saveMyUuid(any))
        .thenAnswer((realInvocation) async => throw UnimplementedError("Save my uuid ERROR!!"));
